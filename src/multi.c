  ...
  while ((!flag1) && (!flag2)) {
...

    {
      ...
      if ((bars->error_barrier).bar_teller == (nprocs)) {
        ...
        pthread_cond_wait(&((bars->error_barrier).bar_cond),
                          &((bars->error_barrier).bar_mutex));
      }
    ...
    if (local_err > global_err_multi) {
      global_err_multi = local_err;
    }
